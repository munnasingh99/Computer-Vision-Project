...
def assignments(descriptors, clusters):
    ...
    matches = bf.match(descriptors, clusters)  
    
    
    # create hard assignment
    # TODO
    T = np.shape(descriptors)[0]  
    K = np.shape(clusters)[0]
    assignment = np.zeros((T, K))  # (T,K)
    for i in range(T):
        assignment[i, matches[i].trainIdx] = 1 
    return assignment
...
def vlad(files, mus, powernorm, gmp=False, gamma=1000):
    ...
    for f in tqdm(files):
        ...
        for k in range(mus.shape[0]):
            ...
            if T_idx is None:
                ...
                if gmp is False:
                    ...
                    f_enc[k * D:k * D + D] = sum_dis  # frequent encoding, 
                    

        f_enc = f_enc.reshape((1,-1))  # normalization need 2D array as input; (1, D*K)
        ...
        if powernorm:
            ...
            init_encode=normalize_f_enc  
        else:
            init_encode=f_enc
            
        if (len(encodings) == 0):
            encodings = init_encode
        else:
            encodings = np.concatenate((encodings, init_encode), axis=0)
...
def esvm(encs_test, encs_train, C=1000):
    ...
    y_train = np.concatenate(([[1]], -np.ones((M, 1))), axis=0).ravel()  # (M+1,1)
    lsvc = LinearSVC(C=C)
...
def evaluate(encs, labels):
    ...
    with open("exercise3.txt", "a") as f:
        f.write("\n \n" + "d)  ESVM results" + "\n")
...
if __name__ == '__main__':
    ...
    if not os.path.exists(fname) or args.overwrite:
        # TODO
        enc_train = vlad(files_train, mus, powernorm=False, gmp=False, gamma=gamma)
        with gzip.open(fname, 'wb') as fOut:
